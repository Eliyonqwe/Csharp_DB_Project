...
begin transaction
	...
	delete from offer where sellerUserID=@id 
	delete from offer where offeringUserID=@id 
	delete from orders where orderingUserID=@id 
	delete from orders where selleruserID=@id 
	delete from stockListing where userID=@id 
	delete from users where userId = @id

	declare  @errValue int = @@ERROR
		IF @errValue > 0
